(function () {
  ...
  function renderMain() {
    ...
    const screenshotHotkey = draft.interaction?.screenshot_hotkey ?? (draft.interaction?.screenshot_hotkey_enabled === false ? "" : "Ctrl+Alt+S");
    ...
    root.innerHTML = `
        ...
        <section class="window-card">
          ...
          <section class="card result-card ${showResultCard ? "" : "hidden"}">
            ...
            <div class="status-row">
              <div class="status-pill" id="statusText">${escapeHtml(state.ui.status || "就绪")}</div>
            </div>
          </section>
        ...
        <div class="sheet-panel">
          ...
          <div class="settings-scroll" id="settingsScroll" data-preserve-scroll="settings">
            ...
            <section class="setting-group">
              ...
              <div class="field"><label>默认方向</label>
            ...
            <section class="setting-group">
              ...
              ${selectionEnabled ? `
                <div class="field-row">
                  <div class="field">
                    ...
                    ${choiceGroupMarkup("interaction.selection_trigger_mode", selectionTriggerMode, selectionModeOptions)}
                      <option value="icon" ${selectionTriggerMode === "icon" ? "selected" : ""}>图标触发</option>
                      <option value="double_ctrl" ${selectionTriggerMode === "double_ctrl" ? "selected" : ""}>双击 Ctrl</option>
                    </select>
                  </div>
                  <div class="field">
                    <label>图标触发方式</label>
                    <select data-field="interaction.selection_icon_trigger">
                      <option value="click" ${draft.interaction?.selection_icon_trigger === "click" ? "selected" : ""}>点击</option>
                      <option value="hover" ${draft.interaction?.selection_icon_trigger === "hover" ? "selected" : ""}>悬停</option>
                    </select>
                  </div>
                </div>
                ...
                <div class="field">
                  <label>划词快捷键</label>
                  <input class="shortcut-input" data-field="interaction.selection_hotkey" value="${escapeHtml(selectionHotkey)}" placeholder="例如 Ctrl+Alt+T，留空表示禁用" />
                </div>
              ` : ""}
              <div class="field">
                <label>截图快捷键</label>
                <input class="shortcut-input" data-field="interaction.screenshot_hotkey" value="${escapeHtml(screenshotHotkey)}" placeholder="例如 Ctrl+Alt+S，留空表示禁用" />
              </div>
            </section>
          </div>
          <div class="settings-actions">
      ...
      <section class="zoom-modal ${state.zoomOpen ? "open" : ""}">
        ...
        <div class="modal-panel">
          ...
          <div class="zoom-shell" id="zoomShell" data-preserve-scroll="zoom-shell">
            <div class="zoom-grid">
      ...
      <div class="bubble-shell">
        <section class="bubble-card">
          <header class="bubble-header" data-drag-handle="bubble-header">
            <div class="bubble-title">${brandIcon("bubble-title-icon", state.branding?.bubbleIconUrl || state.branding?.appIconUrl, icons.book)}<span class="bubble-title-text">${escapeHtml(bubble.action || "划词翻译")}</span></div>
            <div class="toolbar bubble-toolbar">
              <button class="icon-button bubble-pin ${bubble.pinned ? "active" : ""}" data-action="toggle-pin">${icons.pin}</button>
              <button class="icon-button" data-action="close-app">${icons.close}</button>
            </div>
          </header>
          <div class="bubble-content">
            <div class="bubble-stack bubble-stack-source">
              <div class="bubble-label bubble-source-label">原文</div>
              <div class="bubble-source">${escapeHtml(bubble.source_text || "等待选中文本")}</div>
            </div>
            <div class="bubble-stack bubble-stack-result">
              <div class="bubble-label bubble-result-label">${bubble.pending ? "正在翻译" : "译文"}</div>
              <div class="bubble-result ${bubbleResultClass}">${bubbleResultContent}</div>
            </div>
          </div>
  ...
  function patchMainDynamic() {
    ...
    if (!textarea || !resultPane || !statusText) return false;
  ...
  function patchBubbleDynamic() {
    ...
    const title = document.querySelector(".bubble-title-text");
    const source = document.querySelector(".bubble-source");
    ...
    if (!title || !source || !label || !result || !pin || !state.bubble) return false;

    title.textContent = state.bubble.action || "划词翻译";
    source.textContent = state.bubble.source_text || "等待选中文本";
  ...
  function handleBackendEvent(event, payload) {
    ...
    switch (event) {
      ...
      case "translation-done":
        ...
        if (!state.historyOpen && !state.settingsOpen && !state.zoomOpen && patchMainDynamic()) {
      ...
      case "translation-cancelled":
        ...
        showToast("warning", payload.message || "翻译已取消");
        break;
